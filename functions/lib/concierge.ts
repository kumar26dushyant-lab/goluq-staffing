 ...
 *
 * It answers on the website (functions/api/assistant.ts) and on WhatsApp
 * (functions/api/wa/meta.ts). Keeping the persona, the prices and the honesty
 * rules in a single module is the point: a visitor who is quoted one price in
 * chat and a different one on WhatsApp stops trusting both.
 */
export interface ConciergeEnv extends GeminiEnv {
  DB: D1Database;
}
 ...
 * standing on, or that they are messaging on WhatsApp. Everything else (persona,
 * live prices, owner instructions) is assembled here so no caller can forget a
 * part of it.
 */
